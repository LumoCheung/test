...
public class Encoder {
	...
	private static final Pattern SUCCESS_PATTERN = Pattern.compile(
			"^\\s*video\\:\\S+\\s+audio\\:.+global headers\\:\\S+.*$",
	...
	private MultimediaInfo parseMultimediaInfo(File source,
		...
		try {
			...
			while (true) {
				...
				if (step == 0) {
					//第一步是为了匹配input开头字段，一直遍历，直到匹配成功，否则info会为空
					...
					//匹配Input #0, avi, from '/Users/zlc/Documents/aaa.avi':
					if (m.matches()) {
					...
					//匹配Duration: 00:00:43.33, start: 0.000000, bitrate: 460 kb/s
					//持续时间，一般在input下一行，没有就进行第4步
					...
					//匹配Stream #0:0: Video: h264 (Constrained Baseline) (AVC1 / 0x31435641), yuv420p(progressive), 1366x768, 326 kb/s, 15 fps, 15 tbr, 15 tbn, 30 tbc
					...
					} else if(line.trim().startsWith("Metadata")||line.trim().startsWith("title")){
						//多了两行数据
						//Metadata:
						//title           : Video Stream
						continue;
	...
	private Hashtable parseProgressInfoLine(String line) {
		...
		//用来解析frame=  446 fps=178 q=27.0 size=    1280kB time=00:00:28.84 bitrate= 363.6kbits/s dup=18 drop=0 speed=11.5x
		while (m.find()) {
			if (table == null) {
				table = new Hashtable();
			}
...
//				System.out.println(line);
				...
				if (step == 4) {
					...
					if (line.length() > 0) {
						...
						if (table == null) {
							...
							if(!STACK_PATTERN.matcher(line).matches()){
								lastWarning = line;
							}
						} else {
							if (listener != null) {
			...
			if (lastWarning != null) {
				if (!SUCCESS_PATTERN.matcher(lastWarning).matches()) {
					//正常解析，这个地方不会有值出现，既然出现了，证明前面的解析出现了问题，没有毙掉多余字段
